...
`#region HEADER` and `#endregion HEADER`(!!but leaving the
$script:DSCModuleName and $script:DSCResourceName variables).Replace
it with the following code, add the code inside the function 
`Invoke-TestSetup`. Also update the function `Invoke-TestCleanup`.
Make sure the `Invoke-TestSetup` is called outside the `try`-block.
